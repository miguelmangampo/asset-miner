...
const getDetails = async () => {
  ...
  try {
    ...
    details = await page.evaluate(async ({ Residential, Commercial, Condominium, Townhouse }) => {
      const sanitize = text => (text || '').replaceAll('\n', '').replaceAll('\t', '');
      const delayFunc = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
      const locations = Array.from(
        document.querySelector('optgroup[label="National Capital Region"]').querySelectorAll('option')
      ).map(node => node.value);
      const list = [];
      ...
      return list;
    }, { Residential, Commercial, Condominium, Townhouse });
